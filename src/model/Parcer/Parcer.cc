...
void Parcer::ReadCountTables(Data& data) {
  ...
  if (std::getline(file_, line)) {
    ...
    int count_tables;
    if (!(iss >> count_tables) || count_tables <= 0) {
...
void Parcer::ReadPricePerHour(Data& data) {
  ...
  if (std::getline(file_, line)) {
    std::istringstream iss(line);
    int price_per_hour;
    if (!(iss >> price_per_hour) || price_per_hour <= 0) {
      throw std::runtime_error("Error line: " + iss.str());
    }
...
void Parcer::ReadLogs(Data& data) {
  ...
  while (std::getline(file_, line)) {
    ...
    if (!(iss >> time >> id >> body) || (id > 4) ||
        !(Format::IsValidTime(time)) ||
        (!data.logs.empty() &&
         !Format::CompareTime(data.logs.back().time, time)) ||
        (id != 2 && (iss >> table)) || !(Format::IsValidName(body))) {
      throw std::runtime_error("Error line: " + iss.str());
    }
    if (id == 2) {
